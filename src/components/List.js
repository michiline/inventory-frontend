...
const Item = ({ item, removeItem }) => {
    ...
    const increaseQuantity = async () => {
      try {
        const response = await fetch(`http://localhost:3000/items/inc?name=${item.name}`, {
          method: 'PATCH',
          mode: 'cors',
          cache: 'no-cache',
          credentials: 'omit',
    ...
    const decreaseQuantity = async () => {
      try {
        if (parseInt(quantity) === 1) {
          const response = await fetch(`http://localhost:3000/items?name=${item.name}`, {
            method: 'DELETE',
            mode: 'cors',
            cache: 'no-cache',
            credentials: 'omit',
          ...
          const response = await fetch(`http://localhost:3000/items/dec?name=${item.name}`, {
            method: 'PATCH',
            mode: 'cors',
            cache: 'no-cache',
            credentials: 'omit',
